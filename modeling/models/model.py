...
class Model(nn.Module):
    ...
    def forward(
        ...
        for event_stream_name, event_streams in src_events.items():
        ...
        causal_mask = causal_attention_mask(embedded_target.shape[1]).to(device=embedded_target.device,
                                                                         non_blocking=True)

        # Use the event predictor to generate predictions
        tgt_output = self.event_predictor(
            tgt=embedded_target,
            memory=event_stream_encodings,
        ...
        for mini_batch in range(len(batch_tgt_events)):
            ...
            for event_stream_name in src_events:
                for i, event_stream in enumerate(src_events[event_stream_name]):
                    src_events[event_stream_name][i] = event_stream.to(dtype=torch.int32,
                                                                       device=torch.get_default_device(),
                                                                       non_blocking=True)
            for i, mask in enumerate(masks):
                masks[i] = mask.to(dtype=torch.get_default_dtype(), device=torch.get_default_device(),
                                   non_blocking=True)
            tgt_events = tgt_events.to(dtype=torch.int32, device=torch.get_default_device(),
